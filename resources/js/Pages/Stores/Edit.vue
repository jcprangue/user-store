  ...
  <AppLayout title="Stores">
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        <Link class="text-indigo-400 hover:text-indigo-600" :href="route('user-store.index')">
        User Stores
        </Link>
      ...
      <div class="w-full sm:max-w-xl mx-auto  px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
        <form @submit.prevent="submit">
          ...
          <div class="mt-4">
            ...
            <TextInput id="name" v-model="form.name" type="text" class="mt-1 block w-full"  autocomplete="off" />
...
export default {
  ...
  mounted() {
    ...
    this.form.location = this.store.location
  },
  methods: {
    submit() {
      const self = this
      this.$inertia
        .put(this.route("user-store.update",this.store.id), this.form, {
          preserveScroll: true,
          onError: (errors) => {
            self.errors = errors
          }
